...
describe('provider routing', () => {
  it('only allows official DeepSeek models by default', () => {
    const source = readServerSource()

    expect(source).toContain("type ModelProvider = 'deepseek'")
    ...
    expect(source).not.toContain("type ModelProvider = 'deepseek' |")
    expect(source).not.toContain('FIREWORKS_API_KEY')
    expect(source).not.toContain('INFRON_API_KEY')
    ...
    expect(source).not.toContain('requestInfronContent')
    ...
    expect(source).not.toContain('llm.onerouter.pro')
  ...
  it('does not use official DeepSeek JSON mode because it can return empty content', () => {
    const source = readServerSource()
    const deepSeekBody = source.slice(source.indexOf('async function requestDeepSeekContent'), source.indexOf('async function repairJsonWithModel'))
