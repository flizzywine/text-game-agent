...
export type RoleplayLayer = 'director' | 'narrator' | 'postprocess' | 'all'
...
function directSkillsForLayer(
  ...
  return skills.filter(skill => {
    if (!idSet.has(skill.id)) return false
    if (skill.layer === layer) return true
    return skill.layer === 'all' && layer !== 'director'
  })
}
  ...
  return [
    {
      ...
      expectedOutput: 'JSON: { "intent": "string", "npc": "string", "policy": "string[]", "craft": "string[]", "load": "string[]", "beats": "string[]", "focus": "string[]", "sceneLimits": "string[]", "longRangeUpdate": "object", "mustNotResolve": "string[]", "brief": "string" }',
    },
    {
      layer: 'narrator',
      ...
      expectedOutput: 'JSON: { "draftText": "完整正文草稿" }',
    },
  ]
}
