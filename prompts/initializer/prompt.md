...
`physicalSceneState` 是初始场景时钟，不是人物状态 schema。

只记录会持续限制角色行动的物理场景事实，例如地点、空间结构、载具阶段、天气、光线、门窗/道路/座位/设备状态、封闭/开放状态、公共规则或正在发生的外部物理事件。

必须包含四个字段：

- `currentScene`：当前场景或阶段。
- `nextScene`：自然发展下的下一步场景。
- `transitionRule`：后续生成中何时从当前场景推进到下一步场景，何时允许停留；若停留当前场景，必须给出阻滞原因。
- `currentSceneForbidden`：当前场景禁止发生的事件和动作。

不要把人物心情、人物关系、动作变化、对话内容、剧情进展、世界观摘要、大纲、伏笔或人物状态放进 `physicalSceneState`。
